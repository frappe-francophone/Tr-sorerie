...
@frappe.whitelist()
def get_customers_transferts(my_socite):
...
@frappe.whitelist()
def get_Employee_transferts(my_socite):
    return frappe.db.sql(
        """
        SELECT
            E.current_address,
        ...
        WHERE E.status = 'Active'  AND E.company = %(my_socite)s
        """, 
        {"my_socite": my_socite}, 
        as_dict=1
    )
...
@frappe.whitelist()
def get_Student_transferts(my_socite):
    return frappe.db.sql(
        """
            SELECT
                S.address_line_1,
            ...
            CROSS JOIN (
                ...
                FROM `tabCompany` C 
                WHERE C.company_name = %(my_socite)s
                LIMIT 1
            ) AS C
...
@frappe.whitelist()
def get_Supliers_transferts(my_socite):
    return frappe.db.sql(
        """                   
            SELECT
                S.supplier_name,
...
@frappe.whitelist()
def get_Patients_transferts(my_socite):
    return frappe.db.sql(
        """
